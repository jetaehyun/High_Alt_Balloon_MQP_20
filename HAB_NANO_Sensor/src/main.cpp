...
int const O3_SENSOR_PIN = A2 ;
int const CO2_SENSOR_PIN = A6;
...
void loop()
{
  ...
  float UVVolts = UVReading * 4.7 / 1024.0;
  float UV_index = UVVolts * 1.64;
  ...
  int NO2Reading = analogRead(NO2_SENSOR_PIN); // Get raw sensor reading for UV Sensor
  float NO2Resistance = NO2SeriesResistor * ((1203.0 / NO2Reading) -1); //Convert reading into resistance value
  ...
  float CO2Volts = CO2Reading * 4.7 / 1024.0;
  float CO2_index = CO2Volts * 1.64;
  Serial.print ("Raw ADC data: ");
  Serial.print (CO2Reading);
  Serial.print ("  CO2 Index (inconplete): ");
  Serial.println (CO2_index);

  int O3Reading = analogRead(O3_SENSOR_PIN); // Get raw sensor reading for O3 Sensor
  float O3Volts = O3Reading * 4.7 / 1024.0;
  float O3_index = O3Volts * 1.64;
  Serial.print ("Raw ADC data: ");
  Serial.print (O3Reading);
  Serial.print ("  O3 Index (inconplete): ");
  Serial.println (O3_index);
